    ...
    <Panel title="Create Course">
        ...
        <div className="sm:col-span-2">
          ...
          <div className="grid gap-2 rounded-xl border border-slate-200 bg-slate-50/50 p-3 sm:grid-cols-2">
            {departments.map((d) => (
              <label
                key={d.id}
                className="flex cursor-pointer items-center gap-2 rounded-lg bg-white px-3 py-2 text-sm ring-1 ring-slate-200 transition hover:ring-emerald-300"
              >
                <input
                  type="checkbox"
      ...
      {selectedDepartmentIds.size > 0 && programOptions.length === 0 ? (
        <p className="mt-4 text-sm text-amber-800">
          ...
          <Link href="/admin/programs/add" className="font-semibold text-emerald-700 hover:underline">
            Add a program
          </Link>{" "}
          first.
        </p>
